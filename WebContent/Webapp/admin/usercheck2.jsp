  ...
  <head>
	...
	//获取用户信息
		List<String> username = new ArrayList<String>();
	  		List<String> userIDcard = new ArrayList<String>();//身份证号
	  		List<String> userphone = new ArrayList<String>();
			List<String> userlicense = new ArrayList<String>();//身份证照
			
			User userlist = (User)request.getAttribute("userinfo");
			
			if(userlist.getIdentityCard()==null)
				userlicense.add("null");
			else
				userlicense.add( userlist.getIdentityCard());
			if(userlist.getRealname()==null)
				username.add( "null");
			else
				username.add( userlist.getRealname().toString());
			if(userlist.getIdentity()==null)
				userIDcard.add( "null");
			else
				userIDcard.add( userlist.getIdentity().toString());
...
<form name="form1" action="<%=path%>/web/administator/examineuser" method="post">
	...
	<button class="btn btn-primary" onClick="javascript:document.form1.submit();"><i class="icon-ok"></i> 通过</button>
...
<form name="form2" action="<%=path%>/web/administator/examineuser" method="post">
	...
	<input type="hidden" name="result" value="-1">
	<input type="hidden" name="userclass" value="3">
	<button class="btn btn-primary" onClick="javascript:document.form2.submit();"><i class="icon-remove"></i> 不通过</button>
